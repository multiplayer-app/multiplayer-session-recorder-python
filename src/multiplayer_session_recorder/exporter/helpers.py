...
def filter_spans_by_trace_id(spans_data):
    try:
        filtered_spans_data = type(spans_data)()  # Create empty instance of same type
        ...
        for resource_span in spans_data.resource_spans:
            ...
            for scope_span in resource_span.scope_spans:
                ...
                if filtered_scope_span.spans:  # Only add if there are filtered spans
                    filtered_resource_span.scope_spans.extend([filtered_scope_span])
            
            if filtered_resource_span.scope_spans:  # Only add if there are filtered scope spans
                filtered_spans_data.resource_spans.extend([filtered_resource_span])
        
        return filtered_spans_data
    except (IndexError, AttributeError):
        return spans_data  # Return original if filtering fails


def filter_logs_by_trace_id(logs_data):
    try:
        filtered_logs_data = type(logs_data)()  # Create empty instance of same type
        ...
        for resource_log in logs_data.resource_logs:
            ...
            for scope_log in resource_log.scope_logs:
                ...
                if filtered_scope_log.log_records:  # Only add if there are filtered logs
                    filtered_resource_log.scope_logs.extend([filtered_scope_log])
            
            if filtered_resource_log.scope_logs:  # Only add if there are filtered scope logs
                filtered_logs_data.resource_logs.extend([filtered_resource_log])
        
        return filtered_logs_data
    except (IndexError, AttributeError):
        return logs_data  # Return original if filtering fails
